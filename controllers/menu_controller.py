...
def menu_page():
    menu = Menu.query.all()
    return render_template("/menu.jinja", title="Menu Items", menu=menu)

@menu_blueprint.route("/menu_add_item")
def redirect_m():
    return render_template("/menu_add.jinja", title="Add A Menu item")
...
def menu_add():
    ...
    return render_template("/menu.jinja", title="Menu Items", title_2="Item Added!", menu=add_menu)
...
def menu_delete(id):
    ...
    db.session.commit()
    menu = Menu.query.all()
    return render_template("/menu.jinja", title="Menu Items", title_2="Item Removed!", menu=menu)
